...
### Why I Hold This
PHI-027 established that named artifacts + gates are what actually get executed under delegation; prose has near-zero execution probability. This is the dual: after a locked model pivot, a pre-staged `/goal` implementation prompt still described the OLD model and would have built the wrong thing — faithfully. The invalidation was first captured only in a branch-local resume doc; the gap surfaced only when Colin asked whether the next step was in a git issue. The fix put a stop-warning at the top of the roadmap ticket (the PR-1 issue) — the altitude an implementer actually reads.

### Evidence
- PHI-027 — artifacts (not prose) govern delegated execution; this PHI is its dual for *stale* artifacts (supports).
...
### Known Tensions
Auditing every artifact on every edit is ceremony — the trigger is a *pivot* (model/contract change), not routine edits. And a "check freshness" reminder is itself prose (near-zero execution per PHI-027) unless mechanized: a lint diffing the launch artifact against the current spec, or a freshness stamp the launcher gates on.
