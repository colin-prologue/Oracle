...
.decisions/
  ...
  obs/
    ...
    OBS-011-edge-cases-section-as-deferral-bucket.md
    ...
...
- [x] Build an MCP `oracle_query` server wrapping the daemon's recall endpoint; tool description carries the same broad trigger phrasing as the CC `/oracle` skill, and the JSON response embeds the relevance-gate instruction the calling model applies. Lived at `mcp/oracle-query/server.py` (PEP 723 inline-deps Python script run via `uv`). Verified end-to-end via MCP stdio client probe — tool listing, off-topic query, and on-topic query all return well-formed responses. *(Registration in `~/.codex/config.toml` and `~/.codex/AGENTS.md` was a manual step — block was documented in `mcp/oracle-query/README.md`. Codex desktop needs absolute paths + explicit `cwd` per openai/codex issue #14449.)* **Retired 2026-05-13** (commit `050a662`) after the 003-oracle-workflow-layer deprecation gates passed — `mcp/oracle-query/` no longer exists in the tree. Cross-tool query access now goes through the native Hindsight MCP server (`scripts/mcp_server.py`): `hindsight_oracle_query` (recall + relevance gate + audit logging), `hindsight_retain_phi`/`hindsight_retain_obs` (capture), `hindsight_log_query` (query audit records).
...
- [x] **Query/answer log** — JSONL append on every `/oracle` invocation (CC) and every `oracle_query` call (MCP) to `${HINDSIGHT_ROOT}/.decisions/queries/YYYY-MM.jsonl`. Empty-result queries logged too (their absence would defeat calibration). CC schema includes the synthesized answer + cited IDs; MCP schema captures available IDs (Codex-side synthesis isn't visible to the server). Both writers now go through the `mcp__hindsight__hindsight_log_query` MCP tool, which anchors the path against `$HINDSIGHT_ROOT` (PHI-006 invariant). Review via `scripts/review_oracle_queries.py [N]`. *Review ritual itself remains manual — no auto-promotion of findings into OBSes (PHI-003 / Pattern 9 autoRetain failure mode).* *(2026-05-13 update: the standalone `oracle_query` MCP writer above is retired; the two writers are now the CC `/oracle` skill, which calls `hindsight_log_query` directly, and the native `hindsight_oracle_query` tool, which logs through `hindsight_log_query` internally on every call — including empty-result and daemon-error queries. Caveat: in the non-empty path the native tool logs *before* the calling model applies the relevance gate, seeding all available IDs as accepted, so a synthesis-side gate rejection is still recorded as relevant; only the CC `/oracle` skill logs post-synthesis accepted/cited IDs. Calibration reads of native-client entries should treat `accepted_ids` as "retrieved", not "judged relevant". The `hindsight_log_query` mechanism and PHI-006 path anchoring remain current.)*
- [ ] **Filesystem `O_EXCL` ID-reservation guard (residual-race close)** — the shipped guards narrow ID collisions but don't close them: the retain tool's collision check (`hindsight_retain_phi/obs`) does `list → check → POST`, which is not atomic, and the skill-prose bump-and-retry is LLM-advisory. The only truly atomic primitive available without new infra is the filesystem: claim the next ID by `O_EXCL`-creating its `.decisions/{phi,obs}/<ID>.md` file (`open(path,"x")` / bash `set -o noclobber`); on failure, bump and retry; only then retain to the bank. **Tension with PHI-009 (write to durable store first):** O_EXCL-as-reservation wants disk-*first*, but the skills retain bank-first by design. Resolution would treat the disk file as a reservation marker (claim stub → bank retain → fill mirror), which is a genuine reordering worth its own decision record. **Deferred** — for a solo user, collisions require two captures fired within ~seconds; the bank guard + prose-bump handle the realistic (sequential/near-sequential) case. Decision trigger: an actual collision that slips past the bank guard. Evidence: OBS-018 (concurrent-sessions hazard), the 2026-06-14 PHI-033/OBS-017/018 collision cleanup.
...
not as an MCP server. Auto-recall and auto-retain are handled by the plugin. *(Scope note, 2026-05-13:
this decision governs the ambient integration path — hooks and skills — not the tool surface.
The standalone `mcp/oracle-query` shim, added later in Phase 7 for Codex cross-tool access,
was retired this day; the Oracle workflow layer now uses the native Hindsight MCP server
(`scripts/mcp_server.py`) as its tool surface for recall, retain, and query logging.
Invocation stays deliberate per PHI-003; MCP is the transport, not an automation layer.)*
